  ...
  return (
    ...
    >
      ...
      <a
        ...
        className={`absolute top-5 z-20 p-2 rounded-lg
      ...
      <div className={`flex flex-col sm:flex-row gap-6${isRTL ? ' sm:flex-row-reverse' : ''}`}>
        {/* Main content */}
        <div className={`flex-1 min-w-0 flex flex-col gap-1 ${isRTL ? 'order-1' : 'order-2'}`}>
          <h3 className="text-xl font-bold text-(--primary-900)
                         mb-2 leading-snug line-clamp-2 text-start">
            {localPaper.title}
          </h3>
          ...
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2
            ...
            {localPaper.journal && (
              <div className="flex items-center gap-1.5 text-xs sm:text-sm
                              px-2 py-0.5 rounded bg-(--primary-50)
          ...
          {localPaper.abstract && (
            <p className="text-(--text-secondary) text-sm mb-2 line-clamp-3 text-start">
              {localPaper.abstract}
            </p>
          )}
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            {localPaper.topics.map((topic) => (
              <span
                ...
                className="px-3 py-1 rounded-full
                           ...
                           text-(--primary-700) text-xs font-semibold
                           border border-(--primary-100)"
              >
                {topic}
              </span>
            ...
            {localPaper.language && (
              <span className="ms-2 px-2 py-0.5 rounded
                               bg-(--gray-200) text-xs
        ...
        <div
          className={`hidden sm:flex flex-col items-center justify-center
            ...
            border-e pe-4 border-(--border-color)`}
        >
          <BookOpen className="w-10 h-10 text-(--primary-400) mb-2" />
          {localPaper.impactFactor != null && (
            <span className="text-xs font-semibold bg-(--primary-100)
                             text-(--primary-800) rounded px-2 py-0.5 min-w-0 flex flex-col items-center text-center leading-tight whitespace-normal max-w-[135px]">
